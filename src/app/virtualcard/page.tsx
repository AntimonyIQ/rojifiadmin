...
export default function VirtualCardPage() {
    ...
    const fetchUsers = async () => {
        try {
            ...
            const url: string = `${Defaults.API_BASE_URL}/admin/user/list?virtualCard=${RequestStatus.PENDING}&page=${pagination.page}&limit=${pagination.limit}${searchParam}`;
    ...
    return (
        <div className="space-y-6 p-4">
            <header className="flex items-center justify-between">
                ...
                <p className="text-sm text-muted-foreground">Admin panel for virtual card requests and management</p>
            </header>
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                            ...
                                            <TableRow>
                                                <TableCell colSpan={6} className="text-center py-6 text-sm text-muted-foreground">
                                                    <div className="flex items-center justify-center gap-2">
                                                        ...
                                                        <span>Loading virtual card requests...</span>
                                                    </div>
                                                </TableCell>
                                            </TableRow>
                                        )}
                                        ...
                                        {!loading && users.length === 0 && (
                                            <TableRow>
                                                <TableCell colSpan={6} className="text-center py-6 text-sm text-muted-foreground">No virtual card requests found.</TableCell>
                                            </TableRow>
                                        )}
                                        ...
                                        {!loading && users.map((u: IUser, idx: number) => (
                                            <TableRow key={u._id || idx}>
                                                ...
                                                <TableCell>
                                                    {(() => {
                                                        const s = u.requested?.virtualCard as RequestStatus | undefined;
                                                        ...
                                                        return (
                                                            <Badge variant={mapVariant(s)} className="capitalize">
                                                                {s ? String(s) : "-"}
                                                            </Badge>
                                                        );
                                                    })()}
