...
const FEE_TYPES = [
  ...
  { id: "percentage_min", label: "% + Min", description: "Percentage fee with minimum amount" },
  { id: "percentage_max", label: "% + Max", description: "Percentage fee with maximum amount" },
  { id: "percentage_min_max", label: "% + Min + Max", description: "Percentage fee with minimum and maximum amount" },
  { id: "range", label: "Range", description: "Different fees for different amount ranges" }
];
...
const rangeFeeSchema = z.object({
  startRange: z.coerce.number().min(0, { message: "Start range must be at least 0" }),
  endRange: z.coerce.number().min(0, { message: "End range must be at least 0" }),
  feeValue: z.coerce.number().min(0, { message: "Fee value must be at least 0" }),
});

// Schema for a single fee configuration
const channelFeeSchema = z.object({
  channelId: z.string(),
  feeType: z.enum(["flat", "percentage_min", "percentage_max", "percentage_min_max", "range"]),
...
export default function FeeConfigForm() {
  ...
  const [expandedCurrency, setExpandedCurrency] = useState<string | null>("NGN");
  ...
  const mutation = useMutation({
    ...
    onError: (error) => {
      toast({
        ...
        description: "There was a problem saving your fee settings. Please try again.",
      });
      setIsLoading(false);
    },
  });
  ...
  function addRange(currencyIndex: number, channelIndex: number) {
    const currentRanges = form.getValues().currencyFees[currencyIndex].channelFees[channelIndex].ranges || [];
  ...
  function removeRange(currencyIndex: number, channelIndex: number, rangeIndex: number) {
    const currentRanges = [...form.getValues().currencyFees[currencyIndex].channelFees[channelIndex].ranges || []];
      ...
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <CardContent className="space-y-6 px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xl font-medium text-gray-900">Fee Configuration</h3>
          ...
          <div className="space-y-4">
            <div className="bg-blue-50 p-4 rounded-md">
              <p className="text-sm text-blue-700">
                <strong>Note:</strong> Click on a currency card to configure fees for different payment channels. 
                Each payment channel can have its own fee type and configuration.
              </p>
            </div>
              ...
              {currencyFields.map((currencyField, currencyIndex) => {
                ...
                return (
                  <Card 
                    ...
                    className={`border overflow-hidden ${expandedCurrency === currencyCode ? 'ring-2 ring-blue-200' : ''}`}
                  >
                    <div 
                      className="flex items-center justify-between p-4 cursor-pointer"
                      onClick={() => setExpandedCurrency(expandedCurrency === currencyCode ? null : currencyCode)}
                    >
                      <div className="flex items-center">
                        ...
                        <div>
                          <div className="text-md font-medium text-gray-900">{currencyCode}</div>
                          <div className="text-sm text-gray-500">{CURRENCIES.find(c => c.code === currencyCode)?.name}</div>
                        </div>
                      </div>
                    ...
                    {expandedCurrency === currencyCode && (
                      <div className="p-4 pt-0 border-t">
                        <Accordion type="single" collapsible className="w-full">
                          {form.getValues().currencyFees[currencyIndex].channelFees.map((channelFee, channelIndex) => {
                            const channel = PAYMENT_CHANNELS.find(c => c.id === channelFee.channelId);
                            
                            return (
                              <AccordionItem value={`channel-${channelFee.channelId}`} key={channelFee.channelId}>
                                <AccordionTrigger className="py-3">
                                  <div className="flex items-center space-x-2">
                                    ...
                                    <FormField
                                      ...
                                      render={({ field }) => (
                                        <FormItem className="flex items-center space-x-2 m-0" onClick={(e) => e.stopPropagation()}>
                                          <FormControl>
                                            <Switch
                                              checked={field.value}
                                ...
                                <AccordionContent className="pt-2">
                                  <div className="space-y-4">
                                    <FormField
                                      ...
                                      render={({ field }) => (
                                        <FormItem>
                                              ...
                                              {FEE_TYPES.map(feeType => (
                                                <SelectItem key={feeType.id} value={feeType.id}>
                                                  <div className="flex flex-col">
                                                    ...
                                                    <span className="text-xs text-gray-500">{feeType.description}</span>
                                                  </div>
                                                </SelectItem>
                                              ))}
                                            </SelectContent>
                                          ...
                                          <FormDescription>
                                            Select the fee structure for this payment channel
                                          </FormDescription>
                                          <FormMessage />
                                        </FormItem>
                                      )}
                                    ...
                                    {form.watch(`currencyFees.${currencyIndex}.channelFees.${channelIndex}.feeType`) === "flat" && (
                                      <FormField
                                        control={form.control}
                                        ...
                                        render={({ field }) => (
                                          <FormItem>
                                            ...
                                            <FormDescription>
                                              Fixed fee amount for each transaction
                                            </FormDescription>
                                            <FormMessage />
                                          </FormItem>
                                        )}
                                    ...
                                    {["percentage_min", "percentage_max", "percentage_min_max"].includes(
                                      form.watch(`currencyFees.${currencyIndex}.channelFees.${channelIndex}.feeType`)
                                    ) && (
                                      <FormField
                                        control={form.control}
                                        ...
                                        render={({ field }) => (
                                          <FormItem>
                                            ...
                                            <FormDescription>
                                              Percentage of transaction amount charged as fee
                                            </FormDescription>
                                            <FormMessage />
                                          </FormItem>
                                        )}
                                    ...
                                    {["percentage_min", "percentage_min_max"].includes(
                                      form.watch(`currencyFees.${currencyIndex}.channelFees.${channelIndex}.feeType`)
                                    ) && (
                                      <FormField
                                        control={form.control}
                                        ...
                                        render={({ field }) => (
                                          <FormItem>
                                            <FormLabel>Minimum Fee Amount</FormLabel>
                                            <FormControl>
                                              <div className="relative">
                                                <span className="absolute left-3 top-2.5 text-gray-500">{currencySymbol}</span>
                                                <Input {...field} className="pl-7" type="number" min="0" step="0.01" />
                                    ...
                                    {["percentage_max", "percentage_min_max"].includes(
                                      form.watch(`currencyFees.${currencyIndex}.channelFees.${channelIndex}.feeType`)
                                    ) && (
                                      <FormField
                                        control={form.control}
                                    ...
                                    {form.watch(`currencyFees.${currencyIndex}.channelFees.${channelIndex}.feeType`) === "range" && (
                                      <div className="space-y-4">
                                        <div className="flex items-center justify-between">
                                          ...
                                          <Button
                                            ...
                                            variant="outline"
                                            size="sm"
                                            onClick={() => addRange(currencyIndex, channelIndex)}
                                            ...
                                            (range, rangeIndex) => (
                                              <div key={rangeIndex} className="border rounded-lg p-4 space-y-4">
                                                <div className="flex justify-between items-center">
                                                  <h4 className="font-medium">Range {rangeIndex + 1}</h4>
                                                  <Button
                                                    type="button"
                                                    variant="ghost"
                                                    size="icon"
                                                    onClick={() => removeRange(currencyIndex, channelIndex, rangeIndex)}
                                                  >
                                                    ...
                                                    render={({ field }) => (
                                                      <FormItem>
                                                        ...
                                                        <FormControl>
                                                          <div className="relative">
                                                            <span className="absolute left-3 top-2.5 text-gray-500">{currencySymbol}</span>
                                                            <Input {...field} className="pl-7" type="number" min="0" step="1" />
                                                          </div>
                                                        </FormControl>
                                                        <FormMessage />
                                                      </FormItem>
